import { logger } from '../config/logger';
import { pool } from '../db/client';
import { findNearestDrivers } from './geo';
import { getIo } from '../realtime/socket';
import { Rooms, ServerEvents } from '../shared/events';
import {
  getDriverUserIds,
  loadRideById,
  setRideStatus,
} from '../modules/rides/rides.service';
import type { Ride, RideNewRequestPayload } from '../shared/types';

const INITIAL_RADIUS_KM = 5;
const DRIVER_LIMIT = 20;

export async function broadcastRideRequest(
  ride: Ride,
  radiusKm: number,
): Promise<number> {
  const nearest = await findNearestDrivers(
    ride.pickupLat,
    ride.pickupLng,
    radiusKm,
    DRIVER_LIMIT,
  );

  if (nearest.length === 0) return 0;

  const driverIds = nearest.map((d) => d.driverId);
  const driverIdToUserId = await getDriverUserIds(driverIds);

  // Surface the rider's name + rating in the dispatch payload so the
  // driver's IncomingRide screen can show real identity instead of a
  // "Rider / 5.0" placeholder.
  const riderRes = await pool.query<{ name: string | null; rating: string }>(
    `SELECT name, rating FROM users WHERE id = $1 LIMIT 1`,
    [ride.riderId],
  );
  const rider = riderRes.rows[0];

  const payload: RideNewRequestPayload = {
    ride,
    suggestedFare: ride.suggestedFare,
    biddingEndsAt: ride.biddingEndsAt,
    riderName: rider?.name ?? null,
    riderRating: rider ? Number(rider.rating) : 5.0,
  };

  const io = getIo();
  let notified = 0;
  for (const driverId of driverIds) {
    const userId = driverIdToUserId.get(driverId);
    if (!userId) continue;
    // Note: the realtime handler is responsible for joining the driver's
    // socket to Rooms.ride(rideId) when they place their first bid — joining
    // requires a live socket reference which we don't have here.
    io.to(Rooms.user(userId)).emit(ServerEvents.RideNewRequest, payload);
    notified += 1;
  }

  return notified;
}

async function markExpansionDone(rideId: string): Promise<void> {
  await pool.query(
    'UPDATE rides SET dispatch_expanded_at = now() WHERE id = $1',
    [rideId],
  );
}

export async function dispatchRide(rideId: string): Promise<void> {
  let ride: Ride;
  try {
    ride = await loadRideById(rideId);
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : String(err);
    logger.error({ err: message, rideId }, 'dispatch: ride not found');
    return;
  }

  if (ride.status !== 'searching') {
    // Already moved on (e.g. cancelled before dispatch ran).
    return;
  }

  const notified = await broadcastRideRequest(ride, INITIAL_RADIUS_KM);

  if (notified > 0) {
    try {
      await setRideStatus(ride.id, 'bidding');
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err);
      logger.warn({ err: message, rideId: ride.id }, 'failed to set status=bidding');
    }
    // Sentinel: drivers were found at 5km, so the radius-expansion poller
    // should not pick this ride up later.
    try {
      await markExpansionDone(ride.id);
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err);
      logger.warn({ err: message, rideId: ride.id }, 'failed to set dispatchExpandedAt');
    }
    return;
  }

  // No drivers in the initial 5km radius. Leave dispatchExpandedAt null so
  // the radiusExpansion poller will pick this ride up after EXPAND_AFTER_MS
  // and re-broadcast at 10km. This is restart-safe.
}
